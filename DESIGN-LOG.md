...
| Q-036 | Should `hooks.json` subscribe to a `PostToolUseFailure` event (or equivalent CC event-name for failed tool calls) so failed-but-attempted actions land in `action-trace.jsonl`? Intent-008 spike empirically observed a failed Bash producing no entry in our PostToolUse-instrumented capture; the oh-my-claudecode hook layer surfaced "PostToolUseFailure:Bash hook additional context" reminders for the same call. Trade-off: failure-trace coverage vs handler-event surface area / potential noise from non-pilot tools. | open | Surfaced by intent-008 spike. Resolution gated on Q-038. |
...
| Q-038 | Does CC's `PostToolUse` event fire on tool failure, or only on success? Intent-008 spike provides n=1 evidence (a single failing Bash `ls /nonexistent-zzzz` produced no entry in `/tmp/post-tooluse-payload.log` while two successful tool calls — one Edit, one Bash — both did). The oh-my-claudecode hook layer reminders distinguished "PostToolUseFailure:Bash" from "PostToolUse:Bash" for the failed-vs-succeeded calls, suggesting CC has distinct events. But n=1 cannot rule out: hook crash/timeout on that specific Bash, per-tool variance (Edit/Write failure modes may differ), per-failure-class variance (file-not-found vs permission-denied vs internal). Resolution unblocks predicate-drop at `bin/tbd.js:199` (currently retained) and decides Q-036. | open | Surfaced by intent-008 spike (n=1); needs multi-tool multi-failure-class probe |
